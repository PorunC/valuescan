...
from config import API_PATH, CHROME_DEBUG_PORT, SEND_TG_IN_MODE_1, SEND_TG_IN_MODE_2
...
def capture_with_existing_browser():
    """
    模式2: 连接到已存在的浏览器实例进行抓包
    """
    # 配置连接选项
    co = ChromiumOptions()
    ...
    try:
        ...
        try:
            for packet in page.listen.steps():
                ...
                logger.info("\n【请求头】")
                try:
                    headers = packet.request.headers if hasattr(packet, 'request') else packet.headers
                    logger.info(json.dumps(dict(headers), indent=2, ensure_ascii=False))
                except Exception as e:
                    logger.warning(f"无法获取请求头: {e}")
                ...
                if packet.response:
                    try:
                        ...
                        logger.info("\n【响应体 - 重要信息提取】")
                        try:
                            # packet.response.body 可能已经是 dict，也可能是 str
                            response_body = packet.response.body
                            if isinstance(response_body, str):
                                response_data = json.loads(response_body)
                            else:
                            ...
                            # 处理响应数据（启用去重，根据全局配置决定是否发送TG）
                            process_response_data(response_data, send_to_telegram=SEND_TG_IN_MODE_2, seen_ids=seen_message_ids)
